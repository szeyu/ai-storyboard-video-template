...
def image_cell(path: str, label: str, exists: bool, required: bool = True) -> str:
    if not path or not exists:
        status = "Missing" if required else "Optional"
        return f'<div class="missing">{status} {html.escape(label)}</div>'
...
def render_html(data: dict, timeline_path: Path, output_path: Path) -> str:
    ...
    for index, item in enumerate(timeline, start=1):
        ...
        rows.append(
            ...
            <tr>
              ...
              <td>{image_cell(last_frame_path, "last frame", last_frame_exists, required=False)}</td>
    ...
    <tr>
      <td colspan="8" class="empty">No timeline scenes found.</td>
    </tr>
    """
    )
